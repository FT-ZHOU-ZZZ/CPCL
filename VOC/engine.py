...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self,
        ...
        OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k = self.state[
            'ap_meter'].overall_topk(3)
        if display:
            if training:
                print('Epoch: [{0}]\t'
                      'Loss {loss:.4f}\t'
...
class CPCLMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self,
                   training,
                   model,
                   criterion,
        ...
        # compute output
        decoupling_distance, coupling_distance = model(feature_var, inp_var)
        self.state['output'] = decoupling_distance - coupling_distance
        self.state['loss'] = criterion(target_var, self.state['output'],
                                       decoupling_distance, coupling_distance)
