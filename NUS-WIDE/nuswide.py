...
def read_image_label(root, file, subset):
    ...
    name = []
    with open(
            os.path.join(root, 'ImageList', 'ImageList',
                         subset + 'Imagelist.txt'), 'r') as f:
        for line in f:
...
def read_object_labels(root, subset):
    path_labels = os.path.join(root, 'nuswide', 'nuswide', 'Groundtruth',
                               'TrainTestLabels')
    labeled_data = dict()
...
class NUSWIDEClassification(data.Dataset):
    ...
    def __init__(self, root, subset, transform=None, inp_name=None):
        self.root = root
        self.path_images = os.path.join(root, 'nuswide', 'nuswide', 'Flickr')
        self.annotation = os.path.join(root, 'nuswide', 'nuswide',
                                       'Groundtruth')
        self.set = subset
        self.transform = transform

        # define filename of csv file
        file_csv = os.path.join(self.annotation,
                                subset + '_classification.csv')
        #
        ...
        print(
            '[dataset] NUS-WIDE classification set=%s number of classes=%d  number of images=%d'
