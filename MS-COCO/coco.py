...
def download_coco2014(root, phase):
    ...
    if not os.path.exists(anno):
        annotations_file = json.load(
            open(
                os.path.join(annotations_data,
                             'instances_{}2014.json'.format(phase))))
...
class COCO2014(data.Dataset):
    ...
    def get_anno(self):
        list_path = os.path.join(self.root, 'data',
                                 '{}_anno.json'.format(self.phase))
    ...
    def get(self, item):
        ...
        img = Image.open(
            os.path.join(self.root, 'data', '{}2014'.format(self.phase),
